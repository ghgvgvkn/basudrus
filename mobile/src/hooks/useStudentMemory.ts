 ...
 * sessions. Mobile twin of `src/features/ai/useStudentMemory.ts` on the
 * web. Backed by the `student_memory` Supabase table with own-only RLS.
 *
 * The hook does three things:
 *   1. Loads the user's memories on mount (most-important first).
 *   2. Exposes add / addMany / remove / update for the Memory UI.
 *   3. Exposes a parseImportPayload helper for the Import flow so the
 *      user can paste a JSON list from ChatGPT/Claude and bulk-insert.
 *
 * UX rules baked in here (same as the web):
 *   - Adding a duplicate fact (case-insensitive) updates the existing
 *     row's importance instead of failing. Mirrors the unique index
 *     on (user_id, lower(fact)).
 ...
 *   - Sanitise: trim, collapse whitespace, enforce 4–600 chars to
 *     match the DB check constraint.
 *
 * Why mobile gets its own copy instead of importing from the web:
 *   - Mobile uses useAuth (AuthContext) while the web uses
 *     useSupabaseSession. The data layer is otherwise identical.
 *   - Mobile bundle doesn't see the `src/` directory.
 */
...
export interface UseStudentMemoryState {
   ...
   *  user, updates that row's importance instead of failing. */
  add: (input: {
    fact: string;
    category?: MemoryCategory;
    importance?: number;
  ...
  update: (
    id: string,
    patch: Partial<Pick<StudentMemoryRow, 'fact' | 'category' | 'importance'>>,
  ) => Promise<boolean>;
...
export function useStudentMemory(): UseStudentMemoryState {
  ...
  const add = useCallback<UseStudentMemoryState['add']>(
    async input => {
  ...
  const addMany = useCallback<UseStudentMemoryState['addMany']>(
    async (inputs, source = 'imported') => {
      ...
      if (rows.length === 0)
        return { ok: false, inserted: 0, error: 'No valid facts to insert.' };

      // Per-row insert (the unique index is on (user_id, lower(fact))
      // which doesn't match a simple upsert key — so we just count
      // rejected dupes silently).
      let inserted = 0;
  ...
  const update = useCallback<UseStudentMemoryState['update']>(
    async (id, patch) => {
      ...
      if (patch.importance !== undefined)
        next.importance = Math.min(10, Math.max(1, patch.importance));
  ...
  const remove = useCallback<UseStudentMemoryState['remove']>(
    async id => {
      if (!userId) return false;
 ...
 * source AI to be prompted with our template (built by buildImportPrompt
 * below) which asks it to output a JSON array of {fact, category,
 * importance} objects.
 *
 * In practice the source AI often wraps the array in prose, markdown
 * fences, or extra text — so this parser is forgiving:
 *   1. Strip common wrappers (```json fences, "Here's the JSON:" prefix)
 *   2. Find the outermost JSON array via bracket counting
 *   3. JSON.parse, retry once with smart-quote / trailing-comma fix
 *   4. Validate each entry; coerce category and importance
 *
...
export function parseImportPayload(raw: string): ParsedImportEntry[] | null {
  ...
  // Step 1: strip common wrappers (code fences, prose prefixes).
  let cleaned = raw.trim();
  cleaned = cleaned.replace(/^```(?:json|JSON)?\s*\n?/m, '');
  cleaned = cleaned.replace(/\n?```\s*$/m, '');
  cleaned = cleaned.replace(/^[^[]*?(?=\[)/s, '');

  // Step 2: find the outermost JSON array via bracket counting.
  ...
  // Step 3: strict parse, then one rescue pass for common AI quirks.
  let parsed: unknown;
  try {
  ...
  for (const entry of parsed) {
    ...
    if (!fact) continue;
    const categoryRaw =
      typeof e.category === 'string' ? e.category.toLowerCase() : 'context';
    const category: MemoryCategory = (ALLOWED.has(categoryRaw as MemoryCategory)
      ? categoryRaw
