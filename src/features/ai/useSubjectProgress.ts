 ...
 *
 * The shape we expose is intentionally small — only what the grid
 * needs (subject, sessions_count, topics covered, weak/strong areas,
 ...
 * don't subscribe to realtime changes — progress is updated by the
 * post-session analyzer (server-side) which writes once per session,
 * so live updates aren't worth the websocket overhead.
 *
...
function computeMastery(sessions: number, strong: number, weak: number): number {
  // Sessions provide a baseline up to ~0.55 at 12 sessions.
  // Diminishing returns — past 12 sessions you need actual mastery
  // signal (strong areas ≫ weak areas) to climb higher.
